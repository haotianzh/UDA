...
def generate_metadata_df(a, b, c, seed):
    np.random.seed(seed)
    dataset_name = '/data/haotian/waterbird/ood_waterbird/embeds'
    metadata_name = 'metadata.csv'
    df = pd.read_csv(os.path.join(dataset_name, metadata_name))
...
def run(a, b, c, seed):
    embeddings = np.load('./embeds/embed_resnet18.npy')
...
if __name__ == "__main__":
    ...
    with open('experiment.pkl', 'wb') as out:
